...
export function getInitialData(): SystemData {
  ...
  const initialData: SystemData = {
    ...
    level: 1,
    currentXP: 0,
    rank: getRankForLevel(1, initialGates),
    streakDays: 0,
    graceTokens: 1,
    ...
    quests: INITIAL_DAILY_QUESTS.map((q) => ({ ...q, completed: false })),
    instantDungeonTasks: INITIAL_INSTANT_DUNGEON_TASKS.map((t) => ({ ...t, completed: false })),
    dailyHistory: [],
    unlockedTitles: ['awakened-hunter'],
    bossFight: {
